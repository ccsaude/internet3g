...
class gui(object):
    ...
    def __handleArgs(self):
        ...
        parser.add_argument("-v", "--version", action="version", version=gui.SHOW_VERSION(),
    ...
    def changeLanguage(self, language):
        ...
        if not PYTHON2:
            ...
            try:
                ...
                self.error(
                    "Translation failed - language file contains errors, ensure there is no whitespace at the beginning of any lines.")
                return
                ...
                for (key, val) in self.config.items(section):
                    ...
                    if len(keys) != 2:
                        ...
                        if keys[1] not in ["prevButton", "nextButton", "title"]:
                            self.warn(
                                "Invalid PAGEDWINDOW label: " + str(keys[1]) + " for PAGEDWINDOW: " + str(keys[0]))
                        else:
                            try:
    ...
    def exception(message):
        gui.logMessage(message, "EXCEPTION")

    @staticmethod
    def critical(message):
        gui.logMessage(message, "CRITICAL")

    @staticmethod
    def error(message):
        gui.logMessage(message, "ERROR")

    @staticmethod
    def warn(message):
        gui.logMessage(message, "WARNING")

    @staticmethod
    def debug(message):
        gui.logMessage(message, "DEBUG")

    @staticmethod
    def info(message):
        gui.logMessage(message, "INFO")
    ...
    def __validateFunctionList(self, functions, mode):
        ...
        if len(functions) != 2:
            raise Exception(
                "Invalid arguments, set<widget>" + mode + "Function requires 1 or 2 functions to be passed in.")
    ...
    def SET_WIDGET_FG(widget, fg, external=False):
        ...
        # handle flash labels
            ...
            widget.origFg = fg
            try:
                widget.config(bg=widget.origBg)
            except:
                pass  # not a flash label
    ...
    def SET_WIDGET_BG(widget, bg, external=False, tint=False):
        ...
        if external or tint:
            ...
            elif widgType in ["Entry", "Text", "AjText", "ScrolledText", "AjScrolledText", "AutoCompleteEntry",
                              "Spinbox"]:
        ...
        if widgType == "Label":
            widget.origBg = bg
            try:
                widget.config(fg=widget.origFg)
            except:
                pass  # not a flash label
    ...
    def cleanseWidgets(self, widget):
        ...
        for k, v in self.WIDGETS.items():
            ...
            if widgType in ["Tab", "Page"]:
                ...
                if widgType == "Pane":
                    widgets = self.n_panes
                elif widgType == "ScrollPane":
                    widgets = self.n_scrollPanes
                elif widgType == "PagedWindow":
                    widgets = self.n_pagedWindows
                elif widgType == "SubWindow":
                    widgets = self.n_subWindows
                elif widgType in ["WidgetBox", "LabelBox", "ButtonBox"]:
                    widgets = self.n_frames
            ...
            if index is None:
                ...
                with PauseCallFunction(callFunction, tick, useVar=False):
                    if value is None:  # then we need to delete it
                        ...
                        gui.debug(
                            "Updating tick: " + str(index) + " from OptionBox: " + str(title) + " to: " + str(value))
                        tick.set(value)
                    else:
                        gui.debug(
                            "Renaming tick: " + str(index) + " from OptionBox: " + str(title) + " to: " + str(value))
    ...
    def __buildButton(self, title, func, frame, name=None):
        ...
        if isinstance(title, list):
            raise Exception(
                "Can't add a button using a list of names: " + str(title) + " - you should use .addButtons()")
    ...
    def xscroll(self, direction, value=None):
        if not self.hscrollbar.hidden:
            if value is not None:
                self.canvas.xview_scroll(direction, value)
            else:
                self.canvas.xview_moveto(direction)

    def yscroll(self, direction, value=None):
        if not self.vscrollbar.hidden:
            if value is not None:
                self.canvas.yview_scroll(direction, value)
            else:
                self.canvas.yview_moveto(direction)
...
class MicroBitSimulator(Frame):
    COLOURS = {0: '#000000', 1: '#110000', 2: '#220000', 3: '#440000', 4: '#660000', 5: '#880000', 6: '#aa0000',
               7: '#cc0000', 8: '#ee0000', 9: '#ff0000'}
    SIZE = 5
...
class GoogleMap(LabelFrame):
    ...
    def __init__(self, parent, app, defaultLocation="Marlborough, UK"):
        ...
        self.terrainOption = OptionMenu(self.canvas, self.terrainType, *self.TERRAINS,
                                        command=lambda e: self.changeTerrain(self.terrainType.get().lower()))
...
class DraggableWidget:
    ...
    def dnd_end(self, target, event):
        ...
        # have been dropped somewhere
            ...
            gui.debug("<<DRAGGABLE_WIDGET.dnd_end>> dropped with Canvas(" + str(self.Canvas) + ") Target=" + str(
                self.dropTarget))
    ...
    def appear(self, canvas, XY):
        ...
        if self.Canvas:
            ...
            self.ID = self.Canvas.create_window(self.X - self.OffsetX, self.Y - self.OffsetY, window=self.Label,
                                                anchor="nw")
